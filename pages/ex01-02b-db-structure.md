...
구현(STEP 1~)에 들어가기 전에, 데이터가 **어디에 어떤 모양으로** 저장되고, **어떻게 넣는지**를 먼저 봅니다. 
...
도메인 모델(1-02)에서 본 `available_copies`(재고), `active_loans_count`(대출 권수), `has_overdue_loan`(연체 여부)은 **이 프로젝트에서는 테이블에 컬럼으로 두지 않습니다.** 저장하는 대신, 조회할 때마다 `loans`에서 계산합니다.
...
> **이건 정답이 아니라 설계 선택입니다.** 성능을 위해 이 값들을 컬럼으로 **저장**(비정규화)하는 방식도 있습니다. 대신 그 경우엔 대출·반납마다 재고 컬럼과 `loans`를 **동기화**해야 하고, 어긋날 위험이 생깁니다.
>
> 이 프로젝트는 반대로 **계산**을 택했습니다. 매번 세는 비용은 있지만, "재고 컬럼과 실제 대출 기록이 어긋나는" 문제가 구조적으로 사라지기 때문입니다. (읽기 트래픽이 아주 크면 저장 방식이 유리할 수 있습니다.)
>
> **리부팅·장애에도 안전합니다.** 파생값은 저장하지 않으니 깨질 것도 없고, 재시작 후 다음 조회 때 `loans`에서 다시 계산됩니다. 디스크에 남는 건 `loans`뿐이고 그 정합성은 DB 트랜잭션이 지킵니다 — 커밋되면 확정되고, 커밋 전에 멈추면 통째로 롤백돼 "재고는 줄었는데 대출 기록은 없는" 반쪽 상태가 남지 않습니다. (저장 방식이라면 카운트 갱신과 대출 삽입을 한 트랜잭션으로 묶지 않을 경우, 재시작 뒤 값이 영구히 어긋날 수 있습니다.)
>
> 실제 계산 구현은 **STEP 4(Adapters)** 에서 다룹니다.
