...
> 이 **엔티티 설계**가 클린 아키텍쳐에서 가장 고민해야할 부분이고, 또 중요한 부분이라고 생각합니다. "규칙을 어느 객체에 담고 무엇을 필드로 둘지"를 정하는 감각은 단번에 얻어지지 않는다고 생각합니다.. 저 역시 [객체지향의 사실과 오해](https://product.kyobobook.co.kr/detail/S000001628109)(조영호, 위키북스)를 참고하며 조금씩 이해를 높여가려고 노력 중입니다. 이 책을 쓰신 조영호님의 내공에 깊은 존경을 표합니다.
...
> - 원본이 **다른 대상(loans)에 있고**, 엔티티가 그 값을 **직접 갱신**해야 함 → 필드로 받아 관리 (`active_loans_count`는 조회 시 채워지고, 대출 때 `register_borrow()`가 +1)
>
> | 규칙 | 필요한 값 | 엔티티 | 원본/파생 |
> |---|---|---|---|
